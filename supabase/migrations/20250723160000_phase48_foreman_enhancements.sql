...
-- Cow-calf shipping (documentary movement — not sales)
...
CREATE INDEX IF NOT EXISTS cow_calf_shipping_herd_idx
  ON public.cow_calf_shipping_records(cow_calf_herd_id)
  WHERE cow_calf_herd_id IS NOT NULL;
...
-- Stocker shipping (lot-level movement without sale)
...
CREATE INDEX IF NOT EXISTS cattle_shipping_group_idx
  ON public.cattle_shipping_records(cattle_group_id);
