...
--
-- PREREQUISITE for treatment stock deduct: RUN_PHASE3B.sql (creates treatment_records)
-- Medicine inventory works even if Phase 3B has not been run yet.
--
-- Recommended order: RUN_PHASE3.sql → RUN_PHASE3B.sql → RUN_PHASE3C.sql

-- ---------------------------------------------------------------------------
-- Medicine items (ranch catalog + on-hand quantity)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Stock adjustment history (treatment FK added below if treatment_records exists)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Link treatments to medicine (only when treatment_records exists)
-- ---------------------------------------------------------------------------
DO $$
BEGIN
  IF EXISTS (
    ...
    END IF;
  ELSE
    RAISE NOTICE 'treatment_records not found — medicine inventory is ready. Run RUN_PHASE3B.sql, then re-run this file to enable treatment stock deduct.';
  END IF;
END $$;
...
-- ---------------------------------------------------------------------------
-- RLS: medicine_stock_adjustments
-- ---------------------------------------------------------------------------
