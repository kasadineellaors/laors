...
function DistributionBar({
  ...
  const colors = ["bg-olive", "bg-olive/70", "bg-olive/45", "bg-charcoal/30"];
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-1 text-xs text-charcoal/70 sm:grid-cols-4">
        {buckets.map((b) => (
          <span key={b.label}>
...
function VerdictBadge({ verdict }: { verdict: string }) {
  const styles: Record<string, string> = {
    matches: "bg-olive/15 text-olive",
    better_than_expected: "bg-olive/20 text-olive",
    worse_than_expected: "bg-rust/15 text-rust",
    insufficient_data: "bg-charcoal/10 text-charcoal/60",
  };
    ...
    <div className="space-y-8">
      {canExport ? (
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-sm font-medium text-charcoal">Export maternal reports</p>
          <p className="mt-1 text-xs text-charcoal/60">CSV or PDF — also available under Setup → Export</p>
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-charcoal">Top performing females</h2>
          <p className="text-sm text-charcoal/60">Ranked by fertility score, lifetime value, and calving consistency</p>
        </div>
        {dashboard.topPerformers.length === 0 ? (
          <p className="text-sm text-charcoal/50">Record breeding and calving to generate fertility scores.</p>
        ) : (
            ...
            {dashboard.topPerformers.map((f) => (
              <li key={f.animalId}>
                <Link
                  ...
                  className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-3 hover:border-olive"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-olive">{f.score}</p>
                    <p className="text-xs text-charcoal/50">Top {f.percentile}%</p>
                  </div>
                </Link>
              </li>
            ))}
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-charcoal">Underperforming females</h2>
          <p className="text-sm text-charcoal/60">Open years, late calving, and low conception</p>
        </div>
        {dashboard.underperformers.length === 0 ? (
          <p className="text-sm text-charcoal/50">No flagged females yet.</p>
        ) : (
          <ul className="space-y-2">
            {dashboard.underperformers.map((f) => (
              <li key={f.animalId}>
                <Link
                  href={`/seedstock/animals/${f.animalId}`}
                  className="flex items-center justify-between rounded-xl border border-rust/30 bg-surface px-4 py-3 hover:border-rust"
                >
                  <div>
                    <p className="font-semibold text-charcoal">{f.tag}</p>
                    <p className="text-xs text-charcoal/60">
                      {f.factors.openYears} open year(s) · {f.factors.calvesBorn} calves born
                    </p>
                  </div>
                  <p className="text-xl font-bold text-rust">{f.score}</p>
                </Link>
              </li>
            ))}
          </ul>
      ...
      <section className="space-y-4 rounded-xl border border-border bg-surface p-4">
        <h2 className="text-lg font-bold text-charcoal">Maternal lifetime value</h2>
        {dashboard.lifetimeValues.slice(0, 8).map((m) => (
          <div key={m.animalId} className="border-b border-border py-3 last:border-0">
            <div className="flex items-start justify-between gap-2">
              <Link href={`/seedstock/animals/${m.animalId}`} className="font-semibold text-olive hover:underline">
                {m.tag}
                {m.name ? ` — ${m.name}` : ""}
              </Link>
              <span className="font-bold text-charcoal">{formatMoney(m.lifetimeValue)}</span>
            </div>
      ...
      {latestDistribution ? (
        <section className="space-y-3 rounded-xl border border-border bg-surface p-4">
          <h2 className="text-lg font-bold text-charcoal">Calving distribution — {latestDistribution.year}</h2>
          <p className="text-sm text-charcoal/60">First, second, and third 21-day periods of the season</p>
      ...
      {dashboard.calvingDistribution.bySireGroup.length > 0 ? (
        <section className="space-y-3">
          <h2 className="text-lg font-bold text-charcoal">By sire group</h2>
          {dashboard.calvingDistribution.bySireGroup.slice(0, 5).map((g) => (
            <div key={`${g.sireKey}-${g.year}`} className="rounded-xl border border-border bg-surface p-4">
              <p className="mb-2 font-medium text-charcoal">
                {g.sireLabel} <span className="text-charcoal/50">({g.year})</span>
              </p>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-bold text-charcoal">Calving ease — expected vs actual</h2>
        {dashboard.sireCalvingEase.length === 0 ? (
          <p className="text-sm text-charcoal/50">Log calving ease scores to validate sire performance.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[520px] text-left text-sm">
              <thead>
                <tr className="border-b border-border text-charcoal/60">
              ...
              <tbody>
                {dashboard.sireCalvingEase.slice(0, 10).map((s) => (
                  <tr key={s.sireKey} className="border-b border-border/60">
                    <td className="py-3 pr-3 font-medium text-charcoal">{s.sireLabel}</td>
      ...
      {latestCrop ? (
        <section className="space-y-3 rounded-xl border border-border bg-surface p-4">
          <h2 className="text-lg font-bold text-charcoal">Calf crop — {latestCrop.year}</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-bold text-charcoal">Cow family performance</h2>
        {dashboard.familyProfiles
          ...
          .map((f) => (
            <Link
              ...
              className="block rounded-xl border border-border bg-surface p-4 hover:border-olive"
            >
              <p className="font-semibold text-charcoal">
                {f.damTag}
                {f.damName ? ` — ${f.damName}` : ""}
