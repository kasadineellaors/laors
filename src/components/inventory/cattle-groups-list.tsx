...
export function CattleGroupsList({ groups }: CattleGroupsListProps) {
  if (groups.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>No cattle groups yet</CardTitle>
          <CardDescription>
            Create a group at a location to start tracking head counts.
          </CardDescription>
        </CardHeader>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-bold text-charcoal">{group.name}</p>
                <p className="text-sm text-charcoal/60">
                  {group.location_breadcrumb ?? "No location assigned"}
                </p>
                {group.ownership_group_name ? (
                  <p className="text-xs text-charcoal/50">{group.ownership_group_name}</p>
                ) : null}
              </div>
